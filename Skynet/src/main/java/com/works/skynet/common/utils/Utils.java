...
public class Utils {
    ...
    public static String strTooLong(String str, int len) {
        ...
        int t_len = 0;
        int i_len = 0;
        for (i_len = 0; i_len < b.length; i_len++) {
            c[i_len] = b[i_len];
            if (b[i_len] < 0) {
                c[i_len + 1] = b[i_len + 1];
                i_len++;
     ...
     *
     * @param str 输入的字符串,格式=yyyy-MM-dd HH:mm:ss.
     ...
     *
     * @param str     输入的字符串
     * @param pattern 输入的字符串日期格式, 缺省为yyyy-MM-dd
     * @return Calendar    返回代表输入字符串的Calendar对象
     */
    ...
    public static String formatMoney(String s) {
        ...
        int i = (int) d;
        if (Utils.isZero(d - i * 1.0)) {
            if (i < 100) {
                return Utils.formatNumber(d, 0);
            }
            int[] m = {
                    100000000,    // 亿
                    10000000,    // 千万
            ...
            for (int t = 0; t < m.length; t++) {
                if (i % m[t] == 0 && (mf[t] || i / m[t] < 10)) {
                    return Utils.formatNumber(i / m[t], 0) + md[t];
                }
            }
            ret = Utils.formatNumber(d, 0);
        } else {
            ret = Utils.formatNumber(d, 2);
        }
        return ret;
    }
     ...
     * @param s    双精度数字
     * @param dot  后跟小数点位数
     * @param type 0-不进行转换,1-百分数计算,2-除10000
     * @return String 返回特定格式的字符串
     */
    ...
    public static String moveDotRight(String s, int step) {
        int idx_dot = s.indexOf(".");//100.11
        String s1 = "", s2 = "";
        if (idx_dot > -1) {//如果有逗号
            s1 = s.substring(0, idx_dot);//s1=100
            s2 = s.substring(idx_dot + 1);//s2=11
        } else {
            s1 = s;
        }
        int len = s2.length();//2
        ...
        String ret = s1 + s2.substring(0, step);//ret = 10011
        ret = parseInt(ret) + "";
        if (s2.length() > step) {//如果还有多余的小数点就拼接上
    ...
    public static String formatValue(String validPat, String value) {
        ...
        if (validPat.equals("currencyPat")) { //金额
            value = Utils.formatNumber((String) value, 2);
            ...
            value = Utils.formatNumber((String) value, 4);

        } else if (validPat.equals("10kPat")) { //万元
            value = Utils.formatNumber((String) value, 2, 2) + "万";

        } else if (validPat.equals("numberPat")) { // 整数
            value = Utils.formatNumber((String) value, 0, 0);
        } else if (validPat.equals("currIntPat")) { // 无小数金额
            value = Utils.formatNumber((String) value, 0);

        } else if (validPat.equals("ratePat")) { //%
            value = Utils.formatNumber((String) value, 4, 1) + "%";
        } else if (validPat.equals("ratePat1")) { //%
            value = Utils.formatNumber((String) value, 4, 1) + "";
        } else if (validPat.equals("rate0Pat")) { //%
            value = Utils.formatNumber((String) value, 0, 1) + "%";

        } else if (validPat.equals("rate2Pat")) { //%
            value = Utils.formatNumber((String) value, 2, 1) + "%";
            ...
            int t = Utils.parseInt((String) value);
            int y = t / 12;
            int m = t % 12;
            if (m <= 0) {
                value = y + "年";
            ...
            value = Utils.formatDate((String) value, "yyyy-MM-dd", "yyyy年M月d日");

        } else if (validPat.equals("dateInputPat")) { //日期
            value = Utils.formatDate((String) value, "yyyy-MM-dd");
            ...
            value = Utils.formatDate((String) value, "yyyyMM", "yyyy年M月");
            ...
            if (value.length() == 3) {
                if ("000".equals(value)) {
                    ...
                    value = Utils.parseInt(value.substring(1)) + "年";
                } else if (value.startsWith("1")) {
                    value = Utils.parseInt(value.substring(1)) + "个月";
                } else if (value.startsWith("2")) {
                    value = Utils.parseInt(value.substring(1)) + "天";
                }
            } else if (value.length() == 2) {
                value = Utils.parseInt(value) + "个月";
            }
        }
    ...
    public static boolean nowInOpenTime(String sysStartTime, String sysCloseTime) {
        ...
        int i = Utils.getSecondTween("2005-01-01 " + Utils.formatDate(currTime, "HH:mm:ss"), "2005-01-01 " + sysStartTime);
        if (i >= 0) {
            int j = Utils.getSecondTween("2005-01-01 " + Utils.formatDate(currTime, "HH:mm:ss"), "2005-01-01 " + sysCloseTime);
            if (j < 0) {
                return true;
            }
        }
     ...
     *
     * @param input   原字符串
     * @param method  掩盖方法：0全部 1从开头 2从结尾
     * @param begin   开始位置
     * @param masklen 掩盖长度
     * @param chinese 是否处理中文
     * @return
     */
...
//("	", true,"utf-8")
	...
	//本程序为测试程序，不要在系统中使用本方法
	public static ReturnSet execSql(String sql){
		ReturnSet rset=null;
		try{
			Element root = new Element("sqls");
			Document document = new Document(root);
			Element element1 = new Element("sql");
			root.addContent(element1);
			element1.setAttribute("sqltxt", sql);
			element1.setAttribute("target", Command.SQL_TARGET);
			EJBHomeFactory homeFactory = EJBHomeFactory.getFactory();
			DaoEjbHome home = (DaoEjbHome) homeFactory.lookupHome(DaoEjbHome.class);
			DaoEjb dao = home.create();
			rset = dao.excute(document);
